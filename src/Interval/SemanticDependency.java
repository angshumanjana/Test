...
public class SemanticDependency{
    ...
    int[][] DependencyArray = null;
        ...
        DependencyArray = new int[totLines][totLines];
          	...
          	//System.out.println("Enter1");
   				...
   				if(!intersection){
   					if(line1!=blockLine){
   						int[][] muse2 = (int[][])struct.use2;
   						int[][] mdef1 = (int[][])struct.def1;
   						int[][] mdef2 = (int[][])struct.def2;
   						int[][] mdef11 = (int[][])struct.def11;

   						int attr = muse2[0].length-1;
   						int abc = 0;
   						if(muse2!=null&&mdef1!=null&&muse2.length==mdef1.length)
   						{
   							xx = 1;
   							for(int i=0;i<muse2.length;i=i+2)
   							{
   								//System.out.println(i + "  " + muse2.length + "  " + attr);
   								int low = max(muse2[i][attr],mdef1[i][attr]);
   								int upp = max(muse2[i+1][attr],mdef1[i+1][attr]);
   						...
   						if(muse2!=null&&mdef2!=null&&muse2.length==mdef2.length)
   						{
   							yy = 1;
   							for(int i=0;i<muse2.length;i=i+2)
   							{
   								int low = max(muse2[i][attr],mdef2[i][attr]);
   								int upp = min(muse2[i+1][attr],mdef2[i+1][attr]);
   								if(low>upp)
   								{
   									yy = 0;
   									break;
	...
	public static boolean overlap(int[][] arr1, int[][] arr2){
		...
		{
			int attr = arr1[0].length -1;
			int cnt = 0;
			for(int i=0;i<arr1.length;i=i+2)
			{				
				if(arr1[i][attr]<=arr2[i][attr] && arr1[i+1][attr]>=arr2[i+1][attr])
					cnt++;
			}
